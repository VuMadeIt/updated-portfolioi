...
test("social icons include GitHub in LinkedIn → Instagram → GitHub order", () => {
  assert.match(
    section,
    /const socialIcons: IconSpecimen\[] = \[[\s\S]*?name: "LinkedIn"[\s\S]*?name: "Instagram"[\s\S]*?name: "GitHub"/,
  );
  assert.doesNotMatch(section, /name: "X"/);
  assert.doesNotMatch(section, /name: "Luma"/);
});
