...
export type CommunityPhoto = {
  ...
  xOffset?: string;
  /** Wider polaroid frame for landscape photos */
  frameVariant?: "default" | "wide";
  /** CSS object-position for cropping focal point */
...
export type CommunityCardData = {
  ...
  /** Optional collage layout variant */
  collageLayout?: "default" | "split-overlap";
};
...
function CommunityLogo({
  ...
  return (
    <div
      className={clsx(
        "relative shrink-0 overflow-hidden rounded-full bg-zinc-100 border border-zinc-200",
        className,
      )}
    >
      <ShimmerImage
        src={src}
        alt={alt}
        className="h-full w-full object-contain p-2.5 md:p-3.5"
...
export default function CommunityCard({ className, data }: CommunityCardProps) {
  ...
  const isSplitOverlapLayout = data.collageLayout === "split-overlap";
  const isMultiRowLayout = !isSplitOverlapLayout && photos.length > 4;

  // Default rotations for photos if not specified
  const defaultRotations = [11, -2, -11, -9, 8, -6, 5];
  ...
  function getPhotoSize(
    ...
    if (photo.orientation === "vertical") return sizeClasses.vertical;
    if (photo.frameVariant === "wide") return sizeClasses.horizontalWide;
    ...
    >
      ...
      {photos.length > 0 && (
        <div className="relative w-full shrink-0 overflow-x-clip px-2 md:px-4">
          ...
          >
            {photos.map((photo, index) =>
              renderPhoto(photo, index, tabletSizeClasses),
            )}
          </div>

          {/* Desktop (xl+): overlap for 4-up, grid for multi-row */}
          <div
            className={clsx(
          ...
          >
            {photos.map((photo, index) => {
              if (isMultiRowLayout) {
                return renderPhoto(photo, index, desktopSizeClasses);
              }
              ...
              <div className="relative flex flex-col items-center gap-1">
                <div className="relative">
                  ...
                  <ShimmerImage
                    ...
                    style={getImageStyle(expandedPhoto)}
                    rounded="rounded-sm"
                  />
                </div>
                {expandedPhoto.caption && (
                  <p
                    className={`mt-6 max-w-[600px] text-center font-['Lucas',sans-serif] text-base tracking-[0.005em] font-normal leading-relaxed text-zinc-600 ${isClosingPhoto ? '' : 'animate-[fadeSlideUp_300ms_ease-out_100ms_both]'}`}
